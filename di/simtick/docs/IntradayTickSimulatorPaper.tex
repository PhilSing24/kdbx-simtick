...
\small \textbf{di.simtick}, \textbf{di.simcalendar} and \textbf{di.simorder} modules for KDB-X}
\date{September 2026}
...
The simulator is made of three KDB-X modules, each building on the one before. \textbf{di.simtick} generates one trading day for one instrument; running it for several instruments gives several independent tapes. \textbf{di.simcalendar} runs it over a calendar of trading days, linking each day to the next. \textbf{di.simorder} generates orders and works them against the simulated market.
...
\end{equation}
with $l$ its leaves. When the near touch moves away from the limit the algorithm re-pegs: a replace to the new touch, behind its displayed size, up to \key{maxreplaces} times, after which the child rests where it is. At expiry, the next child's send time, what is left is cancelled and rolls into the next child; a final aggressive child before the end of the window completes the order. Fill prices sit on the tick or exactly at the midpoint.
...
Each module carries a suite of checks run by a k4unit harness, 48 for the shared configuration, 145 for simtick, 127 for simcalendar and 229 for simorder, that hold the invariants (every trade inside its prevailing quote, spreads whole ticks, quantities conserved, events consistent with fills) and the statistics below.
...
\begin{itemize}
\item \textbf{Top of the book only:} there is no depth, no queue position beyond the displayed size and no cancels by others ahead of a resting child, so passive fills are on the slow side and an aggressive child larger than the displayed size walks exactly one tick.
\item \textbf{The tape does not react to a resting child:} when the market falls through a resting buy, the child fills at its limit on the seller-initiated prints, as it would, but the tape's ask may already sit below that limit. Aggressive fills are always consistent with the quote in force.
...
\midrule
\multicolumn{5}{@{}l}{\textit{Scenario: day-to-day keys (used by di.simcalendar only)}} \\*
