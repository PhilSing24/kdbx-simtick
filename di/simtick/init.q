...
schema[`overnightshare]:("F";`scenario;`calendar;"di.simmarket: share of a trading day's variance that occurs overnight, between 0 and 1 (1 excluded)")
schema[`gapdayweight]:("F";`scenario;`calendar;"di.simmarket: weight of each calendar day beyond the first in an overnight gap's variance")
schema[`regimepersistence]:("F";`scenario;`calendar;"di.simmarket: AR(1) persistence of the day-level regimes, between 0 and 1 (1 excluded)")
schema[`regimecorr]:("F";`scenario;`calendar;"di.simmarket: correlation of the daily shocks to the volatility and volume regimes, between -1 and 1")
schema[`volregimesd]:("F";`scenario;`calendar;"di.simmarket: log spread of the volatility multiplier across days, normalized so the mean daily variance is the configured one")
schema[`volumeregimesd]:("F";`scenario;`calendar;"di.simmarket: log spread of the volume multiplier across days")
