...
- **Aggressive child** (`ordtype` `MKT`): after `latencyms`, it takes what the far touch displays at the far touch and the rest one tick beyond, at the same instant, both fills removing liquidity (`liquidity` `R`). It is `done` at once. A level beyond the order's `limitprice` is not taken: what would have filled there is a `cancel` at once and rolls on.
- **Passive child** (`ordtype` `LMT`): after `latencyms`, a limit at the near touch in force, behind the size displayed there - or at the order's `limitprice` when the touch is beyond it, alone at that level. Every print by the opposite aggressor at or through the limit takes from that queue first, then from the child, at the child's limit (`liquidity` `A`, on the child's venue). When the near touch moves away from the limit the algo re-pegs: a `replace` to the new touch, behind its displayed size, up to `maxreplaces` times, after which the child rests where it is. At expiry (the next child's send time, or `endtime`) what is left is a `cancel`, and rolls into the next child.
...
- **Resting**: after `latencyms` the child rests at the midpoint of the quote in force, invisible, never crossing the spread; a midpoint print beyond the order's `limitprice` does not fill it. There is no re-peg: its limit is always the current midpoint, so the children table shows `ordtype` `PEG`, `pegtype` `MID` and a null `limitprice`.
