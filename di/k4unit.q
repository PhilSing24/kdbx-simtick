...
filepath:{$[10=type x;hsym`$x;-11=type x;hsym x;'"must pass file path as string or symbol"]}
KUstr:{[x:filepath]x 0:DELIM 0:update code:string code from KUTR} / save test results
KUltr:{[x:filepath].z.M.KUTR upsert("SIJSSISIJBBBBZ";enlist DELIM)0:x;.z.m.KUTR} / reload previously saved test results

KUltf:{ / (load test file) - load tests in csv file <x> into KUT
	before:count KUT;
	this:update file:x,action:lower action,lang:`q^lower lang,code:`$code,ms:0^ms,bytes:0j^bytes,repeat:1|repeat,minver:0^minver from `action`ms`bytes`lang`code`repeat`minver`comment xcol("SIJS*IF*";enlist DELIM)0:x:hsym x;
...
KUpexec:{[prefix;lang;code;repeat;allowfail] 
	...
	if[1<VERBOSE;-1 s];$[DEBUG&allowfail;value s;@[value;s;`FA1L]]}
...
KUact:{[action;lang;code;repeat;ms;bytes;file]
	...
	.z.M.KUTR insert(action;ms;bytes;lang;code;repeat;file;msx;bytesx;ok;okms;okbytes;valid;.z.Z);
	}
...
/ === KDB-X Module Interface ===

/ moduletest: find test.csv in module directory and run tests
moduletest:{[modname]
  / find module path
  paths:":"vs getenv`QPATH;
  modpath:"/"sv"."vs string modname;
  testfile:`;
  i:0;
  while[(i<count paths) and testfile~`;
    p:paths[i];
    candidate:hsym`$$[p~"";"";p,"/"],modpath,"/test.csv";
    if[not()~key candidate;testfile:candidate];
    i+:1
    ];
  if[testfile~`;'"test.csv not found for module: ",string modname];
  .z.m.KUT:0#KUT;
  .z.m.KUTR:0#KUTR;
  KUltf testfile;
  KUrt[];
  -1"\n=== Results ===";
  -1"Passed: ",string exec count i from KUTR where ok;
  -1"Failed: ",string exec count i from KUTR where not ok;
  failed:select action,code from KUTR where not ok;
  if[0<count failed;-1"\nFailed tests:";show failed];
  KUTR
  }

/ wrapper functions matching documented API
saveresults:{[path] KUstr path}
loadresults:{[path] KUltr path}
debug:{[val] DEBUG::val}
verbose:{[val] VERBOSE::val}
delim:{[val] DELIM::val}

/ export public interface
export:([moduletest;saveresults;loadresults;debug;verbose;delim])
